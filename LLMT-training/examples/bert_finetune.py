"""BERT fine-tuning example script.

Usage:
    # Single GPU
    python bert_finetune.py --config config.json

    # Multi-GPU with DeepSpeed
    deepspeed --num_gpus=4 bert_finetune.py --deepspeed --deepspeed_config ds_zero2.json
"""
...
def main():
    ...
    if args.config and os.path.exists(args.config):
        ...
        config = {
            "task_code": args.task_code or "local-bert-finetune",
            ...
            "strategy": {"num_gpus": 1},
            "checkpoint": {"save_interval": 500, "checkpoint_dir": "./checkpoints/bert-finetune"},
            "reporting": {},
        }
    ...
    # Build model
    model_type = config_dict.get("model", {}).get("model_type", "bert")
    ...
    state = TrainingState(
        ...
        max_epochs=config_dict.get("hyperparams", {}).get("max_epochs", 3),
    )
